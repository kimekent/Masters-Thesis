...
# Langchain is used since it offers an easy way to test different memory types, such as
# conversation Buffer, Conversation Window Buffer and Conversation Summary
...
testing_prompt_logs_directory = path + r"\logs\answer_generation"
...
# 2. Define chat bot----------------------------------------------------------------------------------
"""
This testing chat bot is specifically designed for evaluating individual chatbot components.
It takes the following parameters: Test type, retriever, prompt, and input size.
-   The test type determines how many steps of the chatbot are executed before returning the results.
    ...
    To test the prompt and input size, documents are retrieved and in a next step these documents are
    used to generate an answer. 
-   The defined retriever, is function that specifies which document embedding store is searched,
...
-   The prompt determines how the input into ChatGPT is structured.
-   The input size determines how many documents are added to the prompt and fed as context to ChatGPT.
    Note: max_input directly effects the max_output, which determines after how many tokens output is cut off. 
    There is no parameter to set the length of the generated output. For this reason the input is capped
    at 1500 tokens to leave enough tokens for response generation and to avoid errors.
-   To test memory, a multi-turn conversation is input into the model as a list of questions.
    ...
    for reference. Retrieved documents are tracked in a list to determine whether they should be appended
    to the prompt or if they have already been included.
"""
...
def main(test, retriever, query=None, max_input=None, question_prompt=None, retriever_prompt=None):
    ...
    if test == "retriever" or test == "prompt_max_input":
        print("im in retreiver and prompt_input_max")
        ...
        testing_retriever_prompt_logs = path + r"\logs\retriever"
        ...
        for question in query:
            ...
            if query_count >= 1:
                ...
                #print("this is a restructured query: " + restructured_query)
            else:
            ...
            words_to_check = ast.literal_eval(
                open_file(path + r"\testing_data\words_to_check_for_adjusted_similarity_score.txt"))
            ...
            for doc in retrieved_documents:
                ...
                # If the retrieved document is a websupport question append to the designated websupport lists
                    ...
                    if doc["metadata"]["question_id"] not in l_websupport_id_history_tracker[:2]:
                        l_websupport_id.append(doc["metadata"]["question_id"])
                        #print("wesupport id: " + str(doc["metadata"]["question_id"]))
                        websupport_question = doc['context']
                        websupport_answer = doc['metadata']['answer']
                        # Format the question and answer
            ...
            #print("number of tokens retriever: "  + str(tokens_retriever))
            ...
            save_file(updated_retriever_prompt, path + r"\testing_prompts\retriever\retriever_prompt.txt")
            ...
            save_file(updated_history, path + r"\testing_prompts\answer_generation_with_history\qa_prompt_with_history.txt")
