...
# When running the bot on MS Teams replace 'actions.py' file content with the content of this file.
...
# For dispaching teams messages from actions
...
# to websupport. This will automatically create a ticket.
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        try:
            ...
            card = cards.card("Deine Anfrage wurde erfolgreich in eine Outlook-E-Mail übertragen. Dort kannst du "
...
# not satisfactory.
    ...
    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        # Create a card with two buttons
        card = cards.card("Falls, ich deine Frage nicht befriedigend beantworten konnte, kann ich dir ein Websupport Ticket erstellen.")
        card["attachments"][0]["content"]["actions"] = [cards.button("Erstelle ein Websupport Ticket mit meiner Frage", "/open_ticket")]
...
# the email, user is given the chance to edit email message.
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        if new_email_message is not None:
            ...
            card = cards.card(f"Das ist der Inhalt des Tickets:\n\n{user_input}\n\nMöchtest du den Inhalt des Mails"
                              f" vor dem Abschicken noch bearbeiten?")
        card["attachments"][0]["content"]["actions"] = [cards.button("Ja", "/email_not_confirmed"),
                                                        cards.button("Nein", "/email_confirmed")]
...
# If user wants to edit message before sending email from within bot, bot prompts for new message.
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        card = cards.card("Bitte gib deine neue Websupport-Nachricht ein.")
        dispatcher.utter_message(json_message=card)
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        if len(intent_ranking) >= 2:
            ...
            if diff_intent_confidence < 0.2:
                if top_intent == "delete_site" and second_intent == "delete_en_site":
                    card = cards.card("Möchtest du Informationen zum Löschen einer Website?")

                    card["attachments"][0]["content"]["actions"] = [
                        cards.button("Nein, gib mir Informationen zum Löschen einer Sprachversion", "/delete_en_site")]

                elif top_intent == "delete_en_site" and second_intent == "delete_site":
                    card = cards.card("Möchtest du Informationen zum Löschen einer englischen Sprachversion einer Website?")
                    card["attachments"][0]["content"]["actions"] = [
                        cards.button("Nein, gib mir Informationen zum Löschen einer Seite", "/delete_site")]
